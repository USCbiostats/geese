...
ans_pred_sim <- replicate(1e5, {
  do.call(rbind, sim_geese(amodel, params))
}, simplify = "array")
...
ans_pred_sim <- predict_geese_simulate(amodel, params, nsim = 100)
...
(ans <- aphylo::prediction_score(
  cbind(as.vector(ans_pred)[is_leaf]),
  cbind(as.vector(ann_obs)[is_leaf])
  ))
